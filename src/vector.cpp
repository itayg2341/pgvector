	...
	if (a->get_dim() != b->get_dim())
		ereport(ERROR,
				(errcode(ERRCODE_DATA_EXCEPTION),
				 errmsg("different vector dimensions %d and %d", a->get_dim(), b->get_dim())));
}
...
vector_out(PG_FUNCTION_ARGS)
{
	...
	for (int i = 0; i < dim; i++)
	{
		...
		AppendFloat(ptr, vector->get_x()[i]);
	}
...
vector_recv(PG_FUNCTION_ARGS)
{
	...
	for (int i = 0; i < dim; i++)
	{
		result->get_x()[i] = pq_getmsgfloat4(buf);
		CheckElement(result->get_x()[i]);
	}
...
vector(PG_FUNCTION_ARGS)
{
	...
	CheckExpectedDim(typmod, vec->get_dim());
...
array_to_vector(PG_FUNCTION_ARGS)
{
	...
	if (ARR_ELEMTYPE(array) == INT4OID)
	{
		for (int i = 0; i < nelemsp; i++)
			result->get_x()[i] = DatumGetInt32(elemsp[i]);
	}
	else if (ARR_ELEMTYPE(array) == FLOAT8OID)
	{
		for (int i = 0; i < nelemsp; i++)
			result->get_x()[i] = DatumGetFloat8(elemsp[i]);
	}
	else if (ARR_ELEMTYPE(array) == FLOAT4OID)
	{
		for (int i = 0; i < nelemsp; i++)
			result->get_x()[i] = DatumGetFloat4(elemsp[i]);
	}
	else if (ARR_ELEMTYPE(array) == NUMERICOID)
	{
		for (int i = 0; i < nelemsp; i++)
			result->get_x()[i] = DatumGetFloat4(DirectFunctionCall1(numeric_float4, elemsp[i]));
	}
	else
	{
		ereport(ERROR,
...
vector_to_float4(PG_FUNCTION_ARGS)
{
	...
	datums = (Datum *) palloc(sizeof(Datum) * vec->get_dim());

	for (int i = 0; i < vec->get_dim(); i++)
		datums[i] = Float4GetDatum(vec->get_x()[i]);

	/* Use TYPALIGN_INT for float4 */
	result = construct_array(datums, vec->get_dim(), FLOAT4OID, sizeof(float4), true, TYPALIGN_INT);
...
halfvec_to_vector(PG_FUNCTION_ARGS)
{
	...
	CheckDim(vec->get_dim());
	CheckExpectedDim(typmod, vec->get_dim());

	result = Vector::Init(vec->get_dim());

	for (int i = 0; i < vec->get_dim(); i++)
		result->get_x()[i] = HalfToFloat4(vec->get_x()[i]);
...
vector_add(PG_FUNCTION_ARGS)
{
	...
	/* Auto-vectorized */
	for (int i = 0, imax = a->get_dim(); i < imax; i++)
		rx[i] = ax[i] + bx[i];
...
vector_concat(PG_FUNCTION_ARGS)
{
	...
	/* Auto-vectorized */
	for (int i = 0, imax = a->get_dim(); i < imax; i++)
		result->get_x()[i] = a->get_x()[i];

	/* Auto-vectorized */
	for (int i = 0, imax = b->get_dim(), start = a->get_dim(); i < imax; i++)
		result->get_x()[i + start] = b->get_x()[i];
...
binary_quantize(PG_FUNCTION_ARGS)
{
	Vector	   *a = PG_GETARG_VECTOR_P(0);
	float	   *ax = a->get_x();
	VarBit	   *result = InitBitVector(a->get_dim());
	unsigned char *rx = VARBITS(result);
	int			i = 0;
	int			count = (a->get_dim() / 8) * 8;
...
subvector(PG_FUNCTION_ARGS)
{
	...
	/*
	 * Check if (start + count > a->get_dim()), avoiding integer overflow. a->get_dim()
	 * and count are both positive, so a->get_dim() - count won't overflow.
	 */
	if (start > a->get_dim() - count)
		end = a->get_dim() + 1;
	else
		end = start + count;
...
vector_accum(PG_FUNCTION_ARGS)
{
	...
	if (newarr)
		...
		CheckExpectedDim(dim, newval->get_dim());
...
vector_avg(PG_FUNCTION_ARGS)
{
	...
	for (int i = 0; i < dim; i++)
	{
		result->get_x()[i] = statevalues[i + 1] / n;
		CheckElement(result->get_x()[i]);
	}
